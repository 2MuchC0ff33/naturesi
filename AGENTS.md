...
checks: no // comments (grep), block comments /_ ... _/ only,
...
scripts/sed/strip-css-comments.sed — remove /\* _/ from CSS
scripts/sed/strip-js-comments.sed — remove // and /_ \*/ from JS
...
CSS rules
Block comments /_ ... _/ only — NEVER // in .css files
...
Rules: - First line: imperative mood, no period, 50 chars max - Body (optional): explain WHY, not WHAT. 72 chars per line. - Footer: reference issues/tickets: "Fixes #42" - Never commit .env with real values - Every commit must pass lint + tests locally before commit
...
Rules: - NEVER amend if the commit is on main or has been pushed - NEVER amend a commit that another developer may have based work on - If a pushed commit is wrong: make a new fix commit, don't amend
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
3j. AUDIT/VALIDATION AUTOMATION & HISTORY
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```sh
# Full audit/validation start
make validate-audit
# Then run standard git checks:
git fsck --full && git status && git branch -a && git worktree list
```

### Git History, Navigation, and Inspection

All standard commands below are required for agent/human workflows. Run before/after significant changes for session transparency and reproducibility.
...
This section merges real-world dual needs:

- Automation first: Audit/validation/traceability (for agent reproducibility, lint/test gating, and full session state capture)
- Manual navigation: for human/agent stepwise session work
  Place audit/validation and Makefile-driven tooling first, per agent best-practice, with manual git inspection below. Everything is agent-parseable, deterministic, and structured for AI/human traceability.
...
Rules: - NEVER rebase commits that have been pushed (except on feature branches
that no one else has based work on) - After rebasing, force-push: git push --force-with-lease - Use --force-with-lease instead of --force (safer — rejects if someone
...
Merge a feature branch into main (user does this on GitHub): # Do NOT merge locally unless user explicitly requests it. # Squash-merge on GitHub is preferred for a clean main history. # User manages PRs — agent only prepares the branch.
...
Use /_ ... _/ only in CSS. Never // in .css files.
...
test/headless/cart-_.test.sh — localStorage, cart count, add to cart
test/headless/checkout-_.test.sh — form population, PayPal redirect
test/headless/payment-_.test.sh — success/cancel flow
test/headless/shipping-_.test.sh — postcode lookup, rate display
...
strip-css-comments.sed — remove /\* _/ from CSS
strip-js-comments.sed — remove // and /_ \*/ from JS
...
Each script must include inline self-test comments: # SELF-TEST: echo "input" | bc -q scripts/bc/script.bc # Expected: <output>
Key scripts:
